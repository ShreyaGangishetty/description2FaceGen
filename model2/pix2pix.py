...
### command to train
### python pix2pix.py --mode train --output_dir  demo_train  --max_epochs 3 --input_dir sketches/Combine_gray --which_direction BtoA
###command to test
###python pix2pix.py --mode test --output_dir demo_test --input_dir sketches/test --checkpoint demo_train
...
EPS = 1e-12
CROP_SIZE = 64  ### for resolution we can also increase the crop size to acccomodate for larger resolution images
...
def load_examples():
    ...
    with tf.name_scope("load_images"):
        ...
        raw_input= tf.image.grayscale_to_rgb(
            raw_input
        )
        raw_input = tf.identity(raw_input)

        raw_input.set_shape([None, None, 3]) ## 3 to 1
        ...
        if a.lab_colorization:
            ...
            b_images = tf.stack([a_chan, b_chan], axis=2) / 50 - 1#110 # color channels with input range ~[-110, 110], not exact
        else:
...
def create_model(inputs, targets):
    ...
    def create_generator(generator_inputs, generator_outputs_channels):
        print("...........generator is  being created")
        layers = []
...
def save_images(fetches, image_dir, step=None):
   # print("...........for saving images")
    filesets = []
    for i, in_path in enumerate(fetches["paths"]):
        name, _ = os.path.splitext(os.path.basename(in_path))
        fileset = {"name": name, "step": step}
        for kind in ["inputs", "outputs", "targets"]:
            fname = "%s-%s.png"%(name,kind)
            #filename = str(name + "-" + kind + ".png").encode('utf-8')
            filename = fname
            if step is not None:
...
def main():
    ...
    examples = load_examples() ## examples are loaded

    print("examples count = %d" % examples.count)
    print("creating a model hence everything  gets initialized here")
    model = create_model(examples.inputs, examples.targets) #### model is created here
    ...
    def deprocess(image):
        ...
        if a.lab_colorization:
            ...
            print("the number of channels of images input-------------------", num_channels)
            if num_channels == 1:
                ...
                L_chan = tf.squeeze((brightness + 1) / 2 * 100, axis=3) ### the axis might needs to be changed
                lab = tf.stack([L_chan, a_chan, b_chan], axis=3) ## even here axis = 3
                rgb = lab_to_rgb(lab) ### need to explore this function .... instead of converting to rgb we can convert to grayscale
    ...
    with sv.managed_session() as sess:
        ...
        if a.mode == "test":
            ...
            # training
            print("----------------traininggg-------------:TRAIN")
            max_steps = 2**32  ### 64 this also needs to be changed may be
